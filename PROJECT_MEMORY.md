...
- **Auto-Git Background Sync (桌面端自动 Git 备份)**:
  - 桌面端保存或点击 LIVE 时，在后台自动完成 `ticker-data.json` 提交与推送，确保 GitHub Pages 和静态 CDN 始终具备最新数据副本。
- **Force Cache Refresh (穿透式缓存刷新)**:
  - 手动点击 LIVE 强制刷新本地缓存并即时触发完整重渲染。
